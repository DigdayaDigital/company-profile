...
export const NavItems = ({ items, className, onItemClick }: NavItemsProps) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "absolute inset-0 hidden flex-1 flex-row items-center justify-center space-x-2 text-base font-medium text-zinc-800 transition duration-200 hover:text-[#ff5100] lg:flex lg:space-x-4",
        className,
      )}
    >
      {items.map((item, idx) => (
        <a
          ...
          onClick={onItemClick}
          className="relative px-4 py-2 text-neutral-800 dark:text-neutral-300 hover:text-[#ff5100] rounded-full cursor-pointer transition-colors"
          key={`link-${idx}`}
          href={item.link}
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 h-full w-full rounded-full bg-orange-100 dark:bg-neutral-800"
            />
          )}
  ...
  return (
    ...
    >
      <img
        ...
        height={40}
      />
      <span className="font-bold text-xl text-gray-800 dark:text-white">Digdaya Digital</span>
    </a>
  );
};
